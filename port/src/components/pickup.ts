...
export class Pickup extends Component {
  ...
  private apply(player: import("../engine/dispatch").Entity): void {
    ...
    switch (this.effect) {
      ...
      // The PLAYER's speed potion is NOT modMoveToLoc #walkSpeedInc (0.075 — that's the CPU path). Merlin
      // routes #walkSpeed → incWalkAcceleration(#potion) = +pPotionAccelerationInc 0.3 to walk ACCELERATION
      // (objPlayerMerlinCharacter.potionCollected → modNavMode). Terminal velocity = accel/0.125, so +0.3
      // accel ⇒ +2.4 on the combat speed cap (the 0.075 was the wrong constant from the wrong path, ~32× low).
      case "speed": player.get(Movement).maxSpeed += 2.4; break;
