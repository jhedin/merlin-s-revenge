# Behavioral Audit: act_stones8

## Classification

**Actor Type:** Cutscene trigger-stone (#chatter subclass)  
**Inherit Chain:** `act_stones8` → `#chatter` → `#actor`  
...
### 1. Actor Definition (casts/data/act_stones8.txt:1-16)

| Property | Value | Semantics |
|----------|-------|-----------|
| `#inherit` | `#chatter` | Inherits chatter parent class |
| `#character` | `#stones8` | Character symbol (sprite identity) |
...
| `#scriptToPerform` | `#stones8` | Named cutscene script to fire on trigger |
| `#inertia` | `100` | Knockback resistance (immobile) |
| `#member` | `anm_stones8_stand_03_01` (gfx) | Sprite member (animation) |
...
| `#collisionRect` | `rect(-60, -2, 60, 2)` | Default rect (overridden by stones8) |
...
### 4. AI Handler: objAiChatter (casts/script_objects/objAiChatter.txt:42-53)

**Update loop — collision detection:**
...
**Trigger dispatch:**
- Each frame, the AI checks `checkForCollisionWithPlayer()` (line 46, calls objGameObject.txt:271).
- On collision, calls `collected()` (line 47), which fires the cutscene and latches `pPerformed`.

### 5. Cutscene Content (casts/data/scr_stones8.txt:1-37)
...
**Port equivalence:** Cutscene file exists at `/home/user/merlin-s-revenge/port/public/assets/cutscenes/stones8.txt` (lines 1-37, identical to original).

---

## PORT IMPLEMENTATION

### 1. Actor Data (port/src/generated/data.json: act_stones8)

```json
{
  "header": { "name": "act_stones8", "type": "#field" },
  "data": {
    "inherit": "#chatter",
    "character": "#stones8",
    "collisionRect": { "left": -320, "top": -320, "right": 320, "bottom": 320 },
    "initFaceDir": 1,
    "member": { "$member": ["anm_stones8_stand_03_01", "gfx"] },
    "miniMapStatus": "#clr",
    "name": "stones8",
    "inertia": 100,
    "team": "#collectables",
    "scriptToPerform": "#stones8",
...
export function spawnFromSymbol(sym: string, x: number, y: number): Entity | null {
  ...
  if (objType === "#objChatter") return spawnChatter(name, x, y);  // Line 142
  if (rec) return spawnUnit(name, x, y, { animChar: spriteCharOr(name) });
  return null;
}
```

**stones8 dispatch:** Registry resolves `#objChatter` → calls `spawnChatter("stones8", x, y)`. ✓
...
- `scriptToPerform` → reads `d["scriptToPerform"]` → `#stones8`. ✓
...
**For stones8:** Lazy-loads from `/assets/cutscenes/stones8.txt` (or bundled manifest if present). File exists. ✓

### 8. Cutscene Parser (port/src/data/cutscene.ts:103-134)

Parses the scr_stones8 format:
...
**stones8.txt content (port/public/assets/cutscenes/stones8.txt):**
- Lines 1-37 identical to original (casts/data/scr_stones8.txt). ✓
...
|--------|----------|------|--------|
| Reach (rect) | `rect(-320, -320, 320, 320)` | `TRIGGER_REACH = 320` (axis-aligned, L∞ metric) | ✓ **MATCH** |
...
| Team | `#collectables` | Resolved via `registry.resolveActor("stones8")["team"]` → `#collectables`. | ✓ **MATCH** |
...
| Script Name | `#scriptToPerform` = `#stones8` | `scriptToPerform` = `"#stones8"` (hash stripped on fire) | ✓ **MATCH** |
| Script Invocation | `g.cutSceneMaster.playCutScene(#stones8)` | `game.scene?.playInGameCutScene("stones8")` | ✓ **FUNCTIONAL MATCH** |
...
| Aspect | Original | Port | Parity |
|--------|----------|------|--------|
| File Path | `casts/data/scr_stones8.txt` | `port/public/assets/cutscenes/stones8.txt` | ✓ **PRESENT** |
...
7. **Script invocation:** `game.scene?.playInGameCutScene("stones8")` on first overlap. ✓
8. **Cutscene file:** Present at `/home/user/merlin-s-revenge/port/public/assets/cutscenes/stones8.txt`. ✓
...
- No animation swaps needed (stones8 has no separate talking art; port comment acknowledges this). ✓
...
Actor stones8 exhibits **100% behavioral parity** with the original Lingo game:

1. ✓ Correctly classified as trigger-stone (chatter subclass, #objChatter).
2. ✓ Trigger reach (320px) and non-solid status match.
3. ✓ Cutscene fire (#scriptToPerform:#stones8) on player overlap.
4. ✓ One-fire latch prevents repeat performance.
5. ✓ Cutscene file (scr_stones8) ported and available.
...
| File | Line(s) | Content |
|------|---------|---------|
| Original: `casts/data/act_stones8.txt` | 1-16 | Actor definition (inherit, team, scriptToPerform, collisionRect) |
...
| Original: `casts/script_objects/objAiChatter.txt` | 42-53 | `update()` method (collision trigger dispatch) |
| Original: `casts/data/scr_stones8.txt` | 1-37 | Cutscene content (dialogue, characters) |
| Port: `port/src/generated/data.json` | act_stones8 | Ported actor config (identical structure) |
...
| Port: `port/src/data/cutscene.ts` | 80-96, 103-134 | Cutscene loader & parser (lazy-load stones8.txt) |
| Port: `port/public/assets/cutscenes/stones8.txt` | 1-37 | Cutscene file (identical to original) |
