...
- **GAP2 (modStretchDeath) — BUILT (2026-06-21).** A `#stretchDeath` unit (act_player + greyGhost) now dies by stretching its body vertically (`scaleY` 1→1.7, anchored at the feet via the reg point) while fading to transparent (`alpha` 1→0) over ~33 frames, instead of switching to a grave / vanishing. Implemented in the `Anim` component (`stretchDeath` cfg from `act_*.#stretchDeath`): `pickAction` keeps the body frame (no grave swap), `update` advances the transform while dead (and resets on extra-life revive), `sprite()` emits `scaleY`+`alpha` (the renderer already supports both). The death *flow* (the 36-frame window → respawn / wasted cutscene) is unchanged — this is purely the visual transform. Covered by `port/test/stretch_death.test.ts`.
- **GAP3 (modStarReleaser + objStar) — BUILT (2026-06-21), and the original audit was CORRECTED.** Re-reading the source: `modStarReleaser`/`starMaster.experienceStar` is NOT a collectible XP economy. `releaseStar` is called from `modExperience.levelUp` (line 207, `pReleaseStarOnLevel` default true) — so it's a **level-up flourish**: a `#experienceStar` particle (act_experienceStar: initVect (0,-2), friction (1,10), `#lifeCount` 30) rises from ANY unit that levels up, drawn just behind it (`setLocZ(locZ-1)`). XP is still awarded on kill exactly as before (that part was never a gap). Implemented as a lightweight particle layer `port/src/render/effects.ts` (`game.effects`): `Experience.attemptLevelUp` spawns a star (not on the `forceLevelUp` re-field path, where the original toggles stars off); the main loop updates + draws it behind the actors; room-change clears it. Covered by `experience.test.ts` (XP-driven level-up releases a star; re-field releases none; star expires after 30 frames).
