...
**SCOPE — the summon-helper feature (in progress).** `#wizard` (summonWizard), `#wizardSelector` (selectNextWizard), and `#army` (summonArmy hotkey) — rescue wizards in-game (`modSummonWizard`/`wizardMaster` `newWizardFound`) then summon/cycle them as helper allies — are being built next.
