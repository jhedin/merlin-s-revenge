...
**SCOPE DECISION (not auto-built) — the summon-helper feature.** `#wizard` (summonWizard), `#wizardSelector` (selectNextWizard), `#army` (summonArmy hotkey), and the `#weaponSelector` palette UI are a genuine, sizable unimplemented feature: rescue wizards in-game (`modSummonWizard`/`wizardMaster` `newWizardFound`), then summon/cycle them as helper allies, plus a click-to-switch weapon palette. The port has the key *names* and instruction text but no runtime actions. This is catalogued (like the character hover-health UI and the gMapBoundary line) for a user scope decision rather than auto-built, because it is a multi-system feature, not a translation gap.
