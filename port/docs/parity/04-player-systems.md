...
| **saveMaster** | Save/load orchestrator: writes `{ver, currentMap, g_potionMaster, g_soundMaster, g_armyMaster}` to a pref; version-gates load. | **PARTIAL** | `systems/save.ts` saves only `{v, room, player-chain}`. Missing per-room actor state, potions, sound, army (see §3). No version gate beyond `v:1`. | L |
...
| **potionMaster** | Tracks potions-drunk per type + on-screen counter; **saves `pPotionsCollected`**. | **MISSING** | No potion-count tracking or persistence at all. Pickup applies effect then vanishes; no `potionCollected` tally. | M |
| **medikitMaster** | Passes display updates between `modMedikit` and the medikit HUD. | **MISSING** | `modMedikit` itself not ported (see §2); display is agent 5. | M |
| **armyMaster** | Reserve army of teleported/summoned units (`pReserveArmy` by team→typ), `createUnit`/`recordUnitDetails`, "NEXT" display; **saves `pReserveArmy`**. | **MISSING** | Core world-persistence feature. Port summons a generic warrior (`spawnAlly`) with no reserve, no per-unit army details, no persistence. | L |
...
|---|---|---|---|---|---|
| medikit | `act_medikit` (`objMedikit`) | `modMedikit.medikitCollected`: +1 medikit; heals 1hp/5-frame counter when below max, consumes medikits. | **PARTIAL/wrong** | Port "heal" = instant full restore; real medikit is a *stockpiled, gradual* heal with HUD + save state (`pNumOfMedikits`, `pRemainingHitpoints`). | M |
| maxikit | `act_maxikit` (`objMedikit`, char `#maxikit`) | Same `objMedikit` path, bigger graphic. | **PARTIAL** | Mapped to same instant heal; same gap as medikit. | S |
...
| **potion counter HUD** | `potionMaster` | Tally + on-screen icons, persisted. | **MISSING** | No per-type count, no display, no save. | M |
...
## 3. Save/load gap list (state not yet persisted)

`saveMaster` top-level shape = `{ ver, currentMap, g_potionMaster, g_soundMaster, g_armyMaster }`.
The port (`systems/save.ts`) persists only `{ v, room (current loc), player-chain }`. Concretely missing:

1. **Per-room actor state (the big one).** `objMap.addSaveData` saves **every room** (`pRooms[]`),
   and each `objRoom` saves `pBeenActivated`, `pRoomCleared`, `pNum`, `pState`, and **`pRoomObjects[]`**
   — every actor's full chain (energy, XP/level, mana stats, medikit stock, residents, construction,
   army-unit details) via each module's `addSaveData`. Port saves **only the player**. On load every
   enemy/dwelling/ally and all room-clear progress is lost.
2. **Room-clear / been-activated flags.** `pRoomCleared`/`pBeenActivated`/`teamMaster.setRoomClear`
   not persisted → reloaded rooms re-spawn cleared content.
3. **Potion tally (`potionMaster.pPotionsCollected`).** Not tracked or saved.
4. **Sound state (`soundMaster.pActive`).** On/off flag not saved.
5. **Army reserve (`armyMaster.pReserveArmy`).** The entire teleport-to-reserve army (per team→typ,
   with per-unit `generateArmyDetails`: level, energy, stats) — not modeled, not saved. This is the
   headline world-persistence feature.
6. **Dwelling / construction state** (`modConstruction.addSaveData`, `modBuilder`,
   `modResidents`/`modArmyUnit`): build progress, residents spawned, reserve contributions — none saved.
7. **Room graves** (`modRoomGraves.addSaveData`) — death markers not persisted.
8. **Medikit stock** (`modMedikit`: `pNumOfMedikits`, `pRemainingHitpoints`, `pMedikitActive`) — the
   port has no medikit stock at all, so nothing to save.
9. **Save-version gate.** Lingo gates load on `sd.ver == pSaveVersion(12)`; port uses `v:1` with no
   real migration/reject path.
