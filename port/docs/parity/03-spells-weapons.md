...
|---|---|---|---|---|---|
| `energyBlast` | Charged bolt; chargeMaxModifier .75/Basic 5/start 0/speed 1; cd 30; `#bullet:energyBlastBullet`; `limitMagic:true`; power .75; gmg 15/5/5 autofire | **YES** (room) | **PARTIAL** | Charge math reproduced in `control.ts` `SPELL`, but damage is a tuned `dmgPerUnit*charge` (not vect·power·mult), `limitMagic` ignored (limiter unbuilt), GMG branch absent, bullet not the real `energyBlastBullet` actor | M |
...
|---|---|---|---|---|
| `modAttack` charge math | capacity/flow/burst → chargeMax/Start/Speed (+limiter, +randomSummon) | **PARTIAL** | `control.ts` reproduces the core formula for energyBlast only; not generic, no per-spell `chargeMax/Modifier/Basic` from data, no `chargeSpeedMax`/`chargeStartMax` clamps | M |
| `modWeaponManager` | weapon registry, cooldown counters (`inc`=agility/dexterity/regen by type), `selectSpell`, `setMultiAttack` | **MISSING** | player has hardcoded punch+one-spell flags; no weapon list, no spell-cycling, no per-weapon cooldown counter | L |
