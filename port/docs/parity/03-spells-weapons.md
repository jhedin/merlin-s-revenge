...
| `energyPulseSpell` | `releaseFunction:#fireBullets`: on release, streams `energyPulse` bullets (`fireDelay 5`, `chargePerUnit 2`) until charge spent; `throwType:#fullstrength` | **YES** (placed in map) | **FAITHFUL** (I8) | `PICKUPS["#energyPulseSpell"]` grants the `#magic` weapon; release latches a stream substate on `PlayerControl` (`tickStream` = `modFireBullets.updateFireBullets`) emitting a `fireSplashBullet` explode bullet every `fireDelay` frames, draining `chargePerUnit` (count = `floor(held/chargePerUnit)`, drain-before-<0). In-browser: a spaced stream (peak 10 concurrent) | L |
| `energyBeamSpell` | Like pulse but `beam:true` → `performBeamAttack` (random-jittered targeted beam); chargePerUnit 5 | no | **FAITHFUL** (I8) | same streaming release; `performBeamAttack` spawns a `Projectile` in beam mode AT the target (±6px jitter), stretched to caster→target dist + rotated (`beamDist`/`beamAngle`), detonating its explode `#attack` on the first frame (a beam line). In-browser: `beamDist 250 / beamAngle -0.14` | L |
...
| `modGoldenMachineGun` + `gmgMaster` | collect→toggle→swap to `gmg*` charge fields, auto-fire, HUD | **FAITHFUL** (I7) | a MODE on `PlayerControl`: `gmgCollected()`/`setGmg()`/`getGmgOn()`; when on, `charge*Of` read the flat `gmg*` fields and the `spellCharged` loop auto-releases+recharges on reaching max (`gmgAutoFire`); collected/on persisted. `gmgMaster` HUD = a cosmetic flag | M |
| `magicLimitMaster` + `objMagicLimit` (`magicLimit`/`1/25/50/75`) | global `getMagicLimit()` scales every `limitMagic` spell's chargeMax | **FAITHFUL** (I1) | `MagicLimitMaster` singleton on `game` (get/set/setDefault, default 100); `charge.ts` reads `game.magicLimit.get()`; the `objMagicLimit` region marker `set(N)` on spawn; `RoomManager.enter` resets to default at room entry (room-scoped). In-browser: works_mr4Demo (6,11) dims the live charge ceiling to 25%, reset on leave | S |
| `objMine` (`fire`/`pitMonster`/5 auras) | static prime→check→detonate; `#explode` `#attack` at its loc; fire/pit damage + re-arm, auras `#takeFreeze` | **FAITHFUL** (I6) | `Mine` component (`components/mine.ts`): FSM calls `resolveSplash` (C2 engine, zero special-casing). fire dies after 10, pitMonster re-arms forever; the 5 auras (dmgMult 0 + `#takeFreeze`) freeze without damage. `teamMaster.findHostileWithin` + team-gating. In-browser: detonate + aura-freeze verified | M |
| `objMusic`/`objTeamOverride`/`objChatter` (region markers + stones) | play track / set gang-up override / talking stones | **FAITHFUL** (I2/I3/I5) | `RegionMarker` plays `audio.playMusic` (key map 1:1, `stopMusic` sentinel) / sets `teamMaster.teamOverride` (room-scoped). Stones spawn as inert decorative sprites (cutscene scripts unbundled, §g.8) | S |
| `objScroll energyPunch` | `#magicMelee` melee-weapon scroll | **FAITHFUL** (I4) | `PICKUPS["#energyPunch"]` → `equipSword`/`addWeapon` (fires through B2). Deviation: `#magicMelee` mana term not applied (B2 melee calibration, §g.6) | S |
...
| `modFireBullets` | streaming release (energyPulse/beam) | **FAITHFUL** (I8) | `PlayerControl.tickStream` mirrors `updateFireBullets`: drain `chargePerUnit` before the <0 check, emit every `fireDelay` frames (counter reset per shot), GMG `fireDelay=0` empties in one tick | M |
